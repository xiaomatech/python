...
class TimeInterval(object):
    ...
    def _isValidTime(self, hour, minute):
        return isinstance(hour, int) and isinstance(
            minute,
...
class XingeApp(object):
    ...
    def ValidateMessageType(self, message):
        if (self.accessId >= self.IOS_MIN_ID and isinstance(message,
                                                            MessageIOS)):
            return True
        elif (self.accessId < self.IOS_MIN_ID and
              not isinstance(message, MessageIOS)):
            return True
        else:
            return False
    ...
    def SetPushParams(self, params, message, environment):
        if False == self.ValidateMessageType(message):
            return False
        if (self.accessId >= self.IOS_MIN_ID and
                environment != self.ENV_PROD and environment != self.ENV_DEV):
            return False
        ...
        if None == msgObj or not isinstance(msgObj, dict):
            ...
            params['message'] = json.dumps(
                msgObj, separators=(',', ':'), ensure_ascii=False)
            return True
    ...
    def PushSingleDevice(self, deviceToken, message, environment=0):
        ...
        if not (isinstance(message, Message) or isinstance(message,
                                                           MessageIOS)):
    ...
    def PushTags(self, deviceType, tagList, tagsOp, message, environment=0):
        ...
        if False == self.SetPushParams(params, message, environment):
            return ERR_PARAM, 'invalid message, check your input', None
        params['device_type'] = deviceType
        params['tags_list'] = json.dumps(
            [str(tag) for tag in tagList], separators=(',', ':'))
    ...
    def QueryPushStatus(self, pushIdList):
        ...
        params = self.InitParams()
        params['push_ids'] = json.dumps(
            [{'push_id': str(pushId)} for pushId in pushIdList],
            separators=(',', ':'))
    ...
    def BatchSetTag(self, tagTokenPairs):
        ...
        params = self.InitParams()
        params['tag_token_list'] = json.dumps(
            [[pair.tag, pair.token] for pair in tagTokenPairs])
...
class XingeHelper(object):
    ...
    def GenSign(cls, path, params, secretKey):
        ...
        signSource = '%s%s%s%s%s' % (cls.HTTP_METHOD, cls.XINGE_HOST, path,
                                     paramStr, secretKey)
    ...
    def Request(cls, path, params):
        httpClient = httplib.HTTPConnection(
            cls.XINGE_HOST, cls.XINGE_PORT, timeout=cls.TIMEOUT)
        if cls.HTTP_METHOD == 'GET':
            httpClient.request(
                cls.HTTP_METHOD, ('%s?%s' % (path, urllib.urlencode(params))),
...
def PushAccountAndroid(accessId, secretKey, title, content, account):
    x = XingeApp(accessId, secretKey)
    return x.PushSingleAccount(0, account, _BuildAndroidNotification(title,
                                                                     content))
