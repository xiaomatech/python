...
class YP:
    ...
    result = smsOperator.single_send({'mobile': '13000000000',
                                      'text': '【yunpian】您的验证码是4444'})
    ...
    result = smsOperator.single_send({'mobile': mobile,
                                      'text': text,
                                      'encrypt': Tea.encrypt_name})
    print json.dumps(result.content, ensure_ascii=False)
    ...
    result = smsOperator.single_send({'mobile': mobile,
                                      'text': text,
                                      'encrypt': DES.encrypt_name})
    print json.dumps(result.content, ensure_ascii=False)

    #
    # 批量短信发送
    print json.dumps(
        smsOperator.batch_send({'mobile': '13000000001,13000000002',
                                'text': '【yunpian】您的验证码是0000'}).content,
        ensure_ascii=False)
    # 个性化短信发送
    print json.dumps(
        smsOperator.multi_send(
            {'mobile': '13000000003,13000000004',
             'text': '【yunpian】您的验证码是4442,【yunpian】您的验证码是4441'}).content,
        ensure_ascii=False)
    ...
    print json.dumps(result.content, ensure_ascii=False)
    print json.dumps(
        tplOperator.get_default({'tpl_id': '2'}).content, ensure_ascii=False)

    # 流量
    flowOperator = FlowOperator()
    print json.dumps(
        flowOperator.recharge({'mobile': '18720085991',
                               'sn': '1008601'}).content,
        ensure_ascii=False)
    ...
    print json.dumps(
        voiceOperator.send({'mobile': '18720085991',
                            'code': '0012'}).content,
        ensure_ascii=False)
