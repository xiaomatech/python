...
class jp:
    _jpush = jpush.JPush(
        jpush_config.get('app_key'), jpush_config.get('master_secret'))
    ...
    push.audience = jpush.audience(
        jpush.tag("tag1", "tag2"), jpush.alias("alias1", "alias2"))
    ...
    push.platform = jpush.all_
    push.smsmessage = jpush.smsmessage("a sms message from python jpush api",
                                       0)
    print(push.payload)
    push.send()
    ...
    schedulepayload = jpush.schedulepayload.schedulepayload("name", True,
                                                            trigger, push)
    ...
    schedule.put_schedule(schedulepayload,
