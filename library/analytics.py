...
class AnalyticsIllegalDataException(AnalyticsException):
    """
...
class AnalyticsNetworkException(AnalyticsException):
    """
...
class AnalyticsDebugException(Exception):
    """
    Debug模式专用的异常
    """
    pass
...
class Analytics(object):
    ...
    class DatetimeSerializer(json.JSONEncoder):
        """
        实现 date 和 datetime 类型的 JSON 序列化，以符合 Analytics 的要求。
        """
    ...
    def __init__(self, consumer=None, project_name=None, enable_time_free = False):
        """
        初始化一个 Analytics 的实例。可以选择使用默认的 DefaultConsumer，也可以选择其它的 Consumer 实现。
    ...
    def _json_dumps(data):
        return json.dumps(data, separators=(',', ':'), cls=Analytics.DatetimeSerializer)
    ...
    def track_signup(self, distinct_id, original_id, properties=None):
        ...
        if original_id is None or len(str(original_id)) == 0:
            raise AnalyticsIllegalDataException("property [original_id] must not be empty")
        if len(str(original_id)) > 255:
            raise AnalyticsIllegalDataException("the max length of property [original_id] is 255")
    ...
    def _normalize_data(data):
        ...
        if data["distinct_id"] is None or len(str(data['distinct_id'])) == 0:
            raise AnalyticsIllegalDataException("property [distinct_id] must not be empty")
        if len(str(data['distinct_id'])) > 255:
            raise AnalyticsIllegalDataException("the max length of property [distinct_id] is 255")
        ...
        if ts_num < 10 or ts_num > 13:
                raise AnalyticsIllegalDataException("property [time] must be a timestamp in microseconds")
        ...
        # 检查 Event Name
        if 'event' in data and not Analytics.NAME_PATTERN.match(data['event']):
            raise AnalyticsIllegalDataException("event name must be a valid variable name. [name=%s]" % data['event'])

        # 检查 Event Name
        if 'project' in data and not Analytics.NAME_PATTERN.match(data['project']):
            raise AnalyticsIllegalDataException("project name must be a valid variable name. [project=%s]" % data['project'])

        # 检查 properties
        if "properties" in data and data["properties"] is not None:
            for key, value in data["properties"].items():
                if not is_str(key):
                    raise AnalyticsIllegalDataException("property key must be a str. [key=%s]" % str(key))
                if len(key) > 255:
                    raise AnalyticsIllegalDataException("the max length of property key is 256. [key=%s]" % str(key))
                if not Analytics.NAME_PATTERN.match(key):
                    raise AnalyticsIllegalDataException(
                        "property key must be a valid variable name. [key=%s]" % str(key))

                if is_str(value) and len(value) > 8191:
                    raise AnalyticsIllegalDataException("the max length of property key is 8192. [key=%s]" % str(key))
...
class DefaultConsumer(object):
    ...
    def _do_request(self, data):
        ...
        try:
            ...
            raise AnalyticsNetworkException(e)
        return True
